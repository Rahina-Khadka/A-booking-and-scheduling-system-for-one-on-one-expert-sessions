    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-bold text-stone-900">Cancel Booking</h2>
        ...
        {wasPaid && (
          <div className={`border rounded-xl p-4 mb-4 ${loading ? 'bg-stone-50 border-stone-200' : policyColor[eligibility?.policy || 'none']}`}>
            {loading ? (
              <p className="text-sm text-stone-500">Checking refund eligibility...</p>
            ) : eligibility ? (
              <>
              ...
              <p className="text-sm text-stone-500">Could not determine refund eligibility.</p>
            )}
          </div>
        )}
        ...
        {!wasPaid && (
          <div className="bg-stone-50 border border-stone-200 rounded-xl p-4 mb-4">
            <p className="text-sm text-stone-600">This booking has no payment — it will simply be cancelled.</p>
          </div>
        )}
        ...
        <div className="mb-4">
          ...
          <textarea
            ...
            className="w-full px-3 py-2 border border-stone-200 rounded-xl text-sm focus:ring-2 focus:ring-red-300 focus:border-transparent outline-none"
          />
        </div>
        ...
        <div className="flex gap-3">
          <button
            onClick={onClose}
            className="flex-1 px-4 py-2 border border-gray-300 rounded-xl text-stone-700 hover:bg-stone-50"
          >
            Keep Booking
          </button>
          <button
