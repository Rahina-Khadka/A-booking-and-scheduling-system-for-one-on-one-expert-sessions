...
const ExpertCard = ({ expert, index = 0 }) => {
  // _id is guaranteed to be a plain string from the backend
  const expertId = String(expert?._id || expert?.id || '').trim();
  if (!expertId || expertId === 'undefined' || expertId === 'null') return null;
    ...
    <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
      ...
      <div className="px-5 pb-5 flex flex-col flex-1">
        ...
        <div className="mt-auto flex gap-2">
          <Link to={`/experts/${expertId}`} className="flex-1 text-center text-sm font-medium px-3 py-2.5 rounded-xl border border-gray-200 text-gray-600 hover:border-indigo-300 hover:text-indigo-600 transition-colors">View Profile</Link>
          <Link to={`/book/${expertId}`} className="flex-1 text-center text-sm font-semibold px-3 py-2.5 rounded-xl bg-gradient-to-r from-indigo-500 to-cyan-500 text-white hover:from-indigo-600 hover:to-cyan-600 transition-all shadow-sm">Book Session</Link>
        </div>
      </div>
    </motion.div>
  );
