...
const BookingCard = ({ booking, onStatusChange, onCancel, isExpert, onReviewSubmitted, onRescheduled }) => {
  ...
  // Payment is required after completion if not yet paid
  const mustPay = !isExpert && booking.status === 'completed' && booking.payment?.status !== 'paid';
  ...
  return (
    <>
      <div className="bg-white rounded-lg shadow-md p-6">
        ...
        {/* Mandatory payment banner after session completion */}
        {mustPay && (
          <div className="mt-4 bg-red-50 border border-red-200 rounded-xl p-3 flex items-start gap-2">
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-red-600 mt-0.5">Your session is complete. Please pay to finalise.</p>
            </div>
          </div>
        )}
        ...
        <div className="mt-4 flex flex-wrap gap-3">
          ...
          {/* Review Button — only if payment is done */}
          {canReview && !mustPay && (
            <button
          ...
          {/* Mandatory payment after completion */}
          {mustPay && (
            <button
          ...
          {isExpert && booking.status === 'pending' && (
            <>
              <button
                onClick={() => onStatusChange(String(booking._id), 'confirmed')}
                className="flex-1 bg-green-500 text-white px-4 py-2 rounded-lg hover:bg-green-600"
              >
                Accept
              </button>
              <button
                onClick={() => onStatusChange(String(booking._id), 'rejected')}
          ...
          {canDownloadInvoice && (
            <button
              onClick={() => invoiceService.downloadInvoice(String(booking._id))}
