                ...
                <motion.div key={bookingId}
                  ...
                  <div className="flex flex-wrap items-start justify-between gap-4">
                    <div className="flex items-center gap-3">
                      <div className="w-11 h-11 rounded-full bg-accent-600 flex items-center justify-center text-white font-bold text-base flex-shrink-0">
                        {b.userId?.name?.charAt(0)?.toUpperCase()}
                      </div>
                      <div>
                        <p className="font-semibold text-stone-900">{b.userId?.name}</p>
                        <p className="text-xs text-stone-400">{b.userId?.email}</p>
                      </div>
                    </div>
                  ...
                  <div className="mt-4 grid grid-cols-2 sm:grid-cols-3 gap-3 text-sm">
                    ...
                    <div className="bg-stone-50 rounded-xl p-3">
                      ...
                      <p className="font-semibold text-stone-800">{b.startTime} – {b.endTime}</p>
                    </div>
                    {b.topic && (
                      <div className="bg-stone-50 rounded-xl p-3 col-span-2 sm:col-span-1">
                        <p className="text-xs text-stone-400 mb-0.5">Topic</p>
                        <p className="font-semibold text-stone-800 truncate">{b.topic}</p>
                      </div>
                    )}
                  </div>
