...
const SessionRoomPage = () => {
  ...
  const initializeSession = async () => {
    try {
      // Fetch all bookings and find the one matching this session
      const bookings = await bookingService.getBookings();
      const currentBooking = bookings.find(b => String(b._id) === String(bookingId));

      // If not found in user's own bookings → not authorized
      if (!currentBooking) {
        setError('Booking not found or you are not a participant.');
        setLoading(false);
        return;
      }

      // User is already authenticated and this booking came from their own /bookings endpoint
      // so no extra participant check needed — the backend already filtered by their ID
  ...
  // Determine the other participant — compare string IDs safely
